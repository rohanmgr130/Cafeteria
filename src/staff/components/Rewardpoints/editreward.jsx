...
function EditReward() {
  ...
  const validateForm = () => {
    ...
    if (!itemData.rewardPoints || itemData.rewardPoints.toString().trim() === '') {
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToast = toast.loading('Updating reward item...');
    ...
    try {
      ...
      // Make sure categories exists and is not undefined before appending
      if (itemData.categories) {
        formData.append('categories', itemData.categories);
      } else {
        formData.append('categories', ''); // Send empty string as fallback
      }
      ...
      const response = await fetch(`${API_BASE_URL}/api/reward-point/update-reward/${id}`, {
        method: 'PUT',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to update reward item: ${response.status}`);
      }
  ...
  const handleGetSingleReward = async () => {
    setIsLoading(true);
    try {
      console.log(`Fetching reward item with ID: ${id}`);
      const response = await fetch(`${API_BASE_URL}/api/reward-point/get-single-reward/${id}`);
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to fetch reward details');
      }

      const data = await response.json();
      console.log("Reward item data received:", data); // Debugging
      ...
      console.error('Error fetching reward details:', error.message);
    ...
    return (
      <div className="flex min-h-screen bg-gray-50 items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-gray-600">Loading reward item...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 ml-16 md:ml-64 p-4">
        <div className="max-w-3xl mx-auto">
          <div className="p-4">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Edit Reward Item</h1>
          </div>
